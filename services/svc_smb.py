...
class SMB_Photos(BaseService):
  SERVICE_NAME = 'SMB-Photos'
  SERVICE_ID = 5

  SUPPORTED_FILESYSTEMS = ['cifs', 'nfs', 'nfs4', 'smbfs', 'ncp', 'ncpfs', 'coda', 'ocfs2', 'gfs', 'pvfs', 'pvfs2', 'fuse.httpfs', 'fuse.curlftpfs']
  ...
  def preSetup(self):
    SMB_Photos.INDEX += 1
    self.smbDir = "/media/smb%d" % SMB_Photos.INDEX
    self.baseDir = os.path.join(self.smbDir, "photoframe")
    ...
    if not os.path.exists(self.baseDir):
      ...
      for device in self.detectAllStorageDevices(onlyMounted=True):
        if device.mountpoint == self.smbDir:
          self.device = device
          logging.info("USB-Service has detected device '%s'" % self.device.device)
          break
      if self.device is None:
        # Service should still be working fine
        logging.warning("Unable to determine which storage device is mounted to '%s'" % self.smbDir)
  ...
  def updateState(self):
    ...
    if not os.path.exists(self.baseDir):
      if not self.mountStorageDevice():
        ...
        self.subState = SMB_Photos.SUBSTATE_NOT_CONNECTED
  ...
  def explainState(self):
    if self._CURRENT_STATE == BaseService.STATE_NO_IMAGES:
      if self.subState == SMB_Photos.SUBSTATE_NOT_CONNECTED:
        return "No storage location (e.g. SMB share) has been detected"
      else:
        return 'Place images and/or albums inside a "photoframe"-directory on your SMB share'
    return None
  ...
  def getMessages(self):
    ...
    if self.device and os.path.exists(self.baseDir):
      ...
      msgs = [
          {
              'level': 'ERROR',
              'message': 'No storage device could be found that contains the "/photoframe/"-directory! Try to reboot or manually mount the desired storage device to "%s"' % self.smbDir,
              'link': None
          }
      ]
    return msgs
          ...
          if 'DEVNAME' in values:
            ...
            if result is not None:
              ...
              for partition in partitions:
                if partition['fstype'] in SMB_Photos.SUPPORTED_FILESYSTEMS:
                  # Final test
                  ...
                  candidate = SMB_Photos.StorageUnit()
  ...
  def mountStorageDevice(self):
    if not os.path.exists(self.smbDir):
      cmd = ["mkdir", self.smbDir]
      try:
    ...
    for candidate in candidates:
      cmd = ['sudo', '-n', 'mount', candidate.device, self.smbDir]
      try:
        subprocess.check_output(cmd, stderr=subprocess.STDOUT)
        ...
        logging.warning('Unable to mount storage device "%s" to "%s"!' % (candidate.device, self.smbDir))
        logging.warning('Output: %s' % repr(e.output))
      self.unmountBaseDir()

    logging.debug("unable to mount any storage device to '%s'" % (self.smbDir))
    return False

  def unmountBaseDir(self):
    cmd = ['sudo', '-n', 'umount', self.smbDir]
    try:
      subprocess.check_output(cmd, stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError:
      logging.debug("unable to UNMOUNT '%s'" % self.smbDir)
